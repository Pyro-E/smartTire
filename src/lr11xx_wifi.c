...
static int wifi_mw_send_results(at_ctx_t *app_ctx)
{
	...
	atcontext->wifi_scan_results.nb_results = 0;
	LOG_INF("at nb_results %u", atcontext->wifi_scan_results.nb_results );

	// add scan results
	for( uint8_t i = 0; i < wifi_results.nbr_results; i++ )
	{
		if (0 != memcmp(&atcontext->wifi_scan_results.mac[i], &wifi_results.results[i].mac_address, WIFI_AP_ADDRESS_SIZE))
		{
			...
			atcontext->wifi_scan_results.nb_results++;
			...
			if (2 == atcontext->wifi_scan_results.nb_results)
			{
				break;
			}
		}
	...
	if (atcontext->wifi_scan_results.nb_results < WIFI_SCAN_NB_AP_MIN)
	{
		atcontext->uplink_type = NOLOC_T;
	}
	else
...
void on_wifi_scan_done(void *arg)
{
	...
	IterateThroughExisting();
}

bool IterateThroughExisting()
{
	static int lNumOfResTried = 0;
	uint8_t i = 0;
	uint8_t j = 0;

	if ((0 == wifi_results.nbr_results) || ((lNumOfResTried + 1) == wifi_results.nbr_results))
	{
		lNumOfResTried = 0;
		return false;
	}
	...
	for(i = lNumOfResTried; i < wifi_results.nbr_results; i++ )
	{
		if ((0 != memcmp(&atcontext->wifi_scan_results.mac[0], &wifi_results.results[i].mac_address, WIFI_AP_ADDRESS_SIZE)) &&
			(0 != memcmp(&atcontext->wifi_scan_results.mac[1], &wifi_results.results[i].mac_address, WIFI_AP_ADDRESS_SIZE)))
		{
			atcontext->wifi_scan_results.rssi[j] = wifi_results.results[i].rssi;
			memcpy( &atcontext->wifi_scan_results.mac[j], &wifi_results.results[i].mac_address, WIFI_AP_ADDRESS_SIZE );
			atcontext->wifi_scan_results.nb_results++;
			j++;
		}
